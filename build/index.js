...
import { z } from "zod";
// Check for API key
const API_BASE_URL = "https://dev.comet.com/opik/api";
const WORKSPACE_NAME = "awkoy";
const API_KEY = "opT28DyIUes6V6CxXTstZ8gNm*eyJiYXNlVXJsIjoiaHR0cHM6Ly9kZXYuY29tZXQuY29tIn0";
if (!API_BASE_URL || !WORKSPACE_NAME || !API_KEY) {
    console.error("Error: API_BASE_URL, WORKSPACE_NAME and API_KEY environment variables are required");
    process.exit(1);
}
...
const makeApiRequest = async (path, options = {}) => {
    ...
    try {
        ...
        if (!response.ok) {
            return {
                data: null,
                error: `HTTP error! status: ${response.status} ${JSON.stringify(response.body)}`,
            };
        }
